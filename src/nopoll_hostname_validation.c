...
{
	int cn_last_pos = -1;
	char * cn_cert_sn = NULL;
	char *cn_data = NULL;
	X509_NAME_ENTRY *cn_entry = NULL;
	ASN1_STRING *cn_asn1_val = NULL;

	/* GET Subject Name of provided server certificate */
	cn_cert_sn = X509_get_subject_name((X509 *) server_cert);
	if (cn_cert_sn == NULL) {
		return ERROR;
	}

	/* Find the position of the CN field in the Subject field of the certificate*/
	cn_last_pos = X509_NAME_get_index_by_NID(cn_cert_sn, NID_commonName, cn_last_pos);
	if (cn_last_pos < 0) {
		return ERROR;
	}

	/* Extract the CN field*/
	cn_entry = X509_NAME_get_entry(cn_cert_sn, cn_last_pos);
	if (cn_entry == NULL) {
		return ERROR;
	}
...
static noPollHostValidationStatus nopoll_cert_subject_alt_name_host_check(const X509 *server_cert, const char *hostname)
{
    ...
    san_gen_names = X509_get_ext_d2i(server_cert, NID_subject_alt_name, NULL, NULL);
